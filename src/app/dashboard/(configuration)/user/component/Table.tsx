...
export default function Table({title}: Readonly<GetData>) {
    ...
    const [dataDetail, setDataDetail] = useState<Partial<User>>({});
    ...
    const getStatus = (status:number) =>{
        if(status == 1)
        {
            return <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs">Aktif</span>;
        }
        else{
            return <span className="bg-green-100 text-red-700 px-2 py-1 rounded-full text-xs">Non Aktif</span>;
        }
    }
    const handlingModal = (action:boolean, type:string, data?:User) => {
        if(type == 'edit' )
        {
            setShowModal(action)
        }else
    ...
    const myData =()=>{
        ...
        return  (dataGrid.map((value:User, key)=>(
                                <tr key={value.id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs">{getStatus(Number(value.status))}</span>
                                    </td>
                                    {action(value)}
                                </tr>
    ...
    const action = (data:User)=>{
        ...
        return(<td className="px-6 py-4 flex gap-2">
            ...
            {data.status ==1?(<button className="text-red-600 hover:underline" onClick={()=>handlingModal(true,"edit",data)} >Non Aktif</button>):(<button className="text-blue-600 hover:underline" onClick={()=>handlingModal(true,"edit",data)} >Aktif</button>)}
