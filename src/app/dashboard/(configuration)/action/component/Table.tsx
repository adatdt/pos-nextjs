...
import type {Action, GetData, SetFiltering} from "../type/type"
...
export default function Table({title}: Readonly<GetData>) {
    ...
    const [dataDetail, setDataDetail] = useState<Partial<Action>>({});
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
            const response = await fetch('/dashboard/action/api', {
                method: 'POST',
                headers: {
    ...
    const handlingModal = (action:boolean, type:string, data?:Action) => {
        if(type == 'edit' )
        {
            setShowModal(action)
        }else
    ...
    const myData =()=>{
        ...
        return  (dataGrid.map((value:Action, key)=>(
