...
import type {DataDelete, DataEdit, DataAdd, DataGrid, SetFiltering} from "../type/type"
...
export const groupService = {
    ...
    async getAllData(paramFitering:SetFiltering) {      
        ...
        const totalData = await prisma.group.count({
            where: whereClause,
        });

        const returnData:DataGrid[] = []
        data.forEach(async element => {
            const id = encryptAES(element.id)
            const row:DataGrid = {id: id, name: element.name, status: element.status}
            returnData.push(row)
        });
